...
public class TransAction {
    ...
    public UnionDeputyTable uninonDepT = mem.loadUnionDeputyTable();
    ...
    public void SaveAll( )
    {
        ...
        mem.saveUnionDeputyTable(uninonDepT);
    ...
    private int Insert(String[] p){
                ...
                if(Condition(attrtype,tuple,attrid,item.deputyrule[2])){
                    ...
                    for(ClassTableItem item3 : classt.classTable){
                        if(item3.classid == classid){
                            ...
                            attrid1[k] = item3.attrid;  //这是第几个属性
                            k++;
                                ...
                                for(ClassTableItem item8: classt.classTable){
                                    if(item8.attrname.equals(item4.deputy)&&Integer.parseInt(item4.rule)!=0){
                                        if(item8.classid==item.deputyid){
                                            int sw = Integer.parseInt(p[3+attrid1[l]]);  //第几个插入的属性
        ...
        //向Union代理类加元组
        for(UnionDeputyTableItem item:uninonDepT.uniondeputyTable){
            if(classid == item.originid){
                //判断代理规则
            ...
            if(classid == item.originid2){
                //判断代理规则

                String attrtype=null;
                int attrid=0;
                for(ClassTableItem item1:classt.classTable){
                    if(item1.classid == classid&&item1.attrname.equals(item.deputyrule2[0])) {
                        attrtype = item1.attrtype;
                        attrid = item1.attrid;
                        break;
                    }
                ...
                if(Condition(attrtype,tuple,attrid,item.deputyrule2[2])){
                    String[] ss= p.clone();
                    String s1 = null;
        ...
        for (Iterator it1 = topt.objectTable.iterator(); it1.hasNext();){
            ...
            if(item.tupleid == id){
                ...
                for(Iterator it = biPointerT.biPointerTable.iterator(); it.hasNext();){
                    ...
                    if(item.tupleid == item1.deputyobjectid){   //直接从代理类里删东西，那条代理记录也不用要了
                        //it.remove();
                      if(!todelete2.contains(item1))
                            todelete2.add(item1);
                    }
                    if(item.tupleid == item1.objectid){  //item是存对象的，item1是存代理的（删的是原类的东西）
                        deobid = item1.deputyobjectid;
                        OandB ob2=new OandB(DeletebyID(deobid));   //递归调用一下
    ...
    private void Drop(String[]p){
        DandUD dud ;  //要删的所有有影响的代理
        dud=Drop1(p);
        for (DeputyTableItem item:dud.d){
            deputyt.deputyTable.remove(item);
        }
        for (UnionDeputyTableItem item1:dud.u){
            uninonDepT.uniondeputyTable.remove(item1);
        }
    }

    private DandUD Drop1(String[] p){
        String classname = p[1];
        int classid = 0;
        ...
        for (Iterator it1 = classt.classTable.iterator(); it1.hasNext();) {
            ...
            if (item.classname.equals(classname) ){
                ...
                for(Iterator it = switchingT.switchingTable.iterator(); it.hasNext();) {
                    ...
                    if ((item2.attr.equals( item.attrname) && (item2.orginclaid == classid))
                            ||(item2.deputy .equals( item.attrname) && (item2.deputyclaid == classid)))
                    {
                       it.remove();
                    }
                }
        ...
        for(ObjectTableItem obj:ob2d.o){
            topt.objectTable.remove(obj);
        }
        for(BiPointerTableItem bip:ob2d.b) {
            biPointerT.biPointerTable.remove(bip);
        }*/
        DandUD dud = new DandUD();
        List<DeputyTableItem> dti = new ArrayList<>();
        List<UnionDeputyTableItem> udti = new ArrayList<>();
        //清deputy
        for(DeputyTableItem item3:deputyt.deputyTable){
            ...
            if(item3.originid == classid){
                ...
                for(String item4: sname){
                        s[1] = item4;
                        DandUD dud1=Drop1(s);
                        for(DeputyTableItem item8:dud1.d){
                            if(!dti.contains(item8))
                                dti.add(item8);
                        }
                        for(UnionDeputyTableItem item8:dud1.u){
                            if(!udti.contains(item8))
                            udti.add(item8);
                        }
                }
                if(!dti.contains(item3))
        ...
        //清UNION DEPUTY
        for(UnionDeputyTableItem item3:uninonDepT.uniondeputyTable){
            if(item3.deputyid == classid){
                if(!udti.contains(item3))
                    udti.add(item3);
            }
            if(item3.originid == classid && item3.originid2!=classid) {
                //删除Union代理类
                if(!udti.contains(item3))
                    udti.add(item3);
                //退化为一个Select代理类（classid不变）
                deputyt.deputyTable.add(new DeputyTableItem(item3.originid2, item3.deputyid, item3.deputyrule2));
            }
            if(item3.originid2 == classid && item3.originid!=classid) {
                //删除Union代理类
                if(!udti.contains(item3))
                    udti.add(item3);
                //整一个新的Select代理类
                deputyt.deputyTable.add(new DeputyTableItem(item3.originid, item3.deputyid, item3.deputyrule));
            }
            if (item3.originid2 == classid && item3.originid==classid){
                //删除Union代理类
                String[]s = p.clone();
                ...
                if (item.classname.equals(bedeputyname)&&item.attrname.equals(p[3+4*i])) {
                    ...
                    bedeputyattrid[i] = item.attrid;
                        ...
                        if(Integer.parseInt(p[4+4*i]) == 1){
                            switchingT.switchingTable.add(new SwitchingTableItem(item.attrname,attrname[i],bedeputyid,classid,p[5+4*i]));
                        }
                        if(Integer.parseInt(p[4+4*i])==0){
                            switchingT.switchingTable.add(new SwitchingTableItem(item.attrname,attrname[i],bedeputyid,classid,"0"));
                        }
                    break;
                }
            };
        ...
        for(ObjectTableItem item3:topt.objectTable){
            if(item3.classid == classid){
                ...
                if(Condition(cattrtype,tuple,cattrid,p[6])){
                    UpdatebyID(item3.tupleid,attrid,p[3].replace("\"",""),classid);
    ...
    private void UpdatebyID(int tupleid,int attrid,String value,int aaclassid){
        for(ObjectTableItem item: topt.objectTable){
                ...
                for(ClassTableItem item4:classt.classTable){
                    if(item4.classid==item1.deputyid){
                        ...
                        for (SwitchingTableItem item5 : switchingT.switchingTable) {
                            ...
                            String dvalue = null;
                            if (item5.attr.equals(attrname) && item5.deputy.equals(dattrname) && item5.orginclaid==aaclassid) {
                                dvalue = value;
                                ...
                                UpdatebyID(item1.deputyobjectid, dattrid, dvalue,item5.deputyclaid);
                                //break;
                            }
                        }
                    }
    ...
    private void Union(String[] p){
        ...
        int classid = 0;
        //第二个SELECT
        String[] attrname2 = new String[attrnumber];
        int[] attrid2 = new int[attrnumber];
        String[] attrtype2= new String[attrnumber];
        String classname2 = p[7+8*attrnumber];
        int classid2 = 0;
        //第一个SELECT
        for(int i = 0;i < attrnumber;i++){
            for (ClassTableItem item:classt.classTable) {
        ...
        for(int i = 0;i < attrnumber;i++){
            for (ClassTableItem item:classt.classTable) {
                if (item.classname.equals(classname2) && item.attrname.equals(p[7+4*attrnumber+4*i])) {
                    classid2 = item.classid;
                    attrid2[i] = item.attrid;
                    attrtype2[i] = item.attrtype;
                    attrname2[i] = p[10+4*attrnumber+4*i];
                    //重命名
        ...
        //第二个SELECT  CONDITION
        int sattrid2 = 0;
        String sattrtype2 = null;
        for (ClassTableItem item:classt.classTable) {
            if (item.classid == classid2 && item.attrname.equals(p[8+8*attrnumber])) {
                sattrid2 = item.attrid;
                sattrtype2 = item.attrtype;
                break;
            }
        }
        ...
        //第二个SELECT
        for(ObjectTableItem item : topt.objectTable){
            if(item.classid == classid2){
                Tuple tuple = GetTuple(item.blockid,item.offset);
                if(Condition(sattrtype2,tuple,sattrid2,p[8*attrnumber+10])){
                    //Switch
                    for(int j = 0;j<attrnumber;j++){
                        if(Integer.parseInt(p[9+4*attrnumber+4*j])==1){
                            int value = Integer.parseInt(p[10+4*attrnumber+4*j]);
                            int orivalue = Integer.parseInt((String)tuple.tuple[attrid2[j]]);
                            Object ob = value+orivalue;
                            tuple.tuple[attrid2[j]] = ob;
                        }
                    }
                    tpl.addTuple(tuple);
                }
        ...
        String[] allattrname = new String[attrnumber*2];
        int[] allattrid = new int[attrnumber*2];
        String[] allattrtype= new String[attrnumber*2];
    ...
    private void CreateUnionDeputy(String[] p){
        ...
        String classname = p[2];//代理类的名字
        String bedeputyname = p[4*count+3];//代理的类的名字
        String bedeputyname2 = p[8*count+8];//代理的类2的名字
        classt.maxid++;
        int classid = classt.maxid;//代理类的id
        int bedeputyid = -1;//代理的类1的id
        ...
        String[] attrtype=new String[count];
        int[] attrid=new int[count];
        String[] attrname2=new String[count];
        String[] bedeputyattrname2=new String[count];
        int[] bedeputyattrid2 = new int[count];
        String[] attrtype2=new String[count];
        int[] attrid2=new int[count];
        ...
        //后一个SELECT的
        for(int j = 0;j<count;j++){
            attrname2[j] = p[4*count+11+4*j];
            attrid2[j] = j;
            bedeputyattrname2[j] = p[4*count+8+4*j];
        }
        ...
        for (int i = 0; i < count; i++) {
            ...
            for (ClassTableItem item:classt.classTable) {
                if (item.classname.equals(bedeputyname)&&item.attrname.equals(p[3+4*i])) {
                    ...
                    classt.classTable.add(new ClassTableItem(classname, classid, count,attrid[i],attrname[i], item.attrtype,"Ude"));
                    //swi
                    if(Integer.parseInt(p[4+4*i]) == 1){
                        switchingT.switchingTable.add(new SwitchingTableItem(item.attrname,attrname[i],bedeputyid,classid,p[5+4*i]));
                    }
        ...
        for (int i = 0; i < count; i++) {
            ...
            for (ClassTableItem item:classt.classTable) {
                if (item.classname.equals(bedeputyname2)&&item.attrname.equals(p[4*count+8+4*i])) {
                    bedeputyid2 = item.classid;
                    bedeputyattrid2[i] = item.attrid;
                    //swi
                    if(Integer.parseInt(p[4*count+9+4*i]) == 1){
                        switchingT.switchingTable.add(new SwitchingTableItem(item.attrname,attrname2[i],bedeputyid2,classid,p[4*count+10+4*i]));
                    }
                    if(Integer.parseInt(p[4*count+9+4*i])==0){
                        switchingT.switchingTable.add(new SwitchingTableItem(item.attrname,attrname2[i],bedeputyid2,classid,"0"));
                    }
                    break;
                }
            };
        ...
        con2[1] = p[10+8*count];
        con2[2] = p[11+8*count];
        uninonDepT.uniondeputyTable.add(new UnionDeputyTableItem(classid,bedeputyid,bedeputyid2,con,con2));
        ...
        for(ObjectTableItem item2:topt.objectTable){
            if(item2.classid ==bedeputyid2){
                Tuple tuple = GetTuple(item2.blockid,item2.offset);
                if(Condition(contype2,tuple,conid2,con2[2])){
                    //插入
                    //swi
                    Tuple ituple = new Tuple();
                    ituple.tupleHeader = count;
                    ...
                    for(int o =0;o<count;o++){
                        if(Integer.parseInt(p[9+4*count+4*o]) == 1){
                            int value = Integer.parseInt(p[10+4*count+4*o]);
                            int orivalue =Integer.parseInt((String)tuple.tuple[bedeputyattrid[o]]);
                            Object ob = value+orivalue;
                            ituple.tuple[o] = ob;
                        }
                        if(Integer.parseInt(p[9+4*count+4*o]) == 0){
    ...
    public TupleList FortSelect(String[] p){
        TupleList tpl = new TupleList();
        int attrnumber = Integer.parseInt(p[1]);
        String[] attrname = new String[attrnumber];
        int[] attrid = new int[attrnumber];
    ...
    private class DandUD{
        public List<DeputyTableItem> d= new ArrayList<>();
        public List<UnionDeputyTableItem> u= new ArrayList<>();
        public DandUD(){}
        public DandUD(DandUD dandUD){
            this.d = dandUD.d;
            this.u = dandUD.u;
        }

        public DandUD(List<DeputyTableItem> d, List<UnionDeputyTableItem> u) {
            this.d = d;
            this.u = u;
        }
    }
    ...
    private void PrintTab(ObjectTable topt,SwitchingTable switchingT,DeputyTable deputyt,BiPointerTable biPointerT,ClassTable classTable,UnionDeputyTable unionDepTable) {
        ...
        bundle0.putSerializable("UnionDepTable",unionDepTable);
    ...
    public void PrintTab(){
        PrintTab(topt,switchingT,deputyt,biPointerT,classt,uninonDepT);
    }
}
